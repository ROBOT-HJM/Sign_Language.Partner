...
class CustomLSTM(nn.Module):
    def __init__(self, input_dim=30, hidden_size=64, num_layers=1, num_classes=15, dropout=0.):
...
def load_dataset_for_lstm(sequence_dir, csv_path, max_seq_length=130):
    ...
    if not {'filename', 'label', 'type'}.issubset(df.columns):
        print(f"CSV 파일에 필요한 열(filename, label, type)이 없습니다.")
        return None, None, None, None
    ...
    for _, row in df.iterrows():
        file_path = os.path.join(sequence_dir, os.path.basename(row['filename']))
        ...
        try:
            ...
            if sequence.shape != (max_seq_length, 30):
                print(f"파일 {file_path}의 형상 {sequence.shape}이 [{max_seq_length}, 30]이 아닙니다.")
                continue
            # 정규화
        ...
        if seq_length < 20:  # 최소 길이 필터링
            print(f"파일 {file_path}의 유효 길이 {seq_length}가 너무 짧습니다.")
            continue
...
def main():
    ...
    print(f"Train labels shape: {train_labels.shape}, Validation labels shape: {val_labels.shape}")
